...
pub fn compile_and_capture(
    ...
    symbolizer: &mut Symbolizer,
    dump: bool,
) -> Result<DebugRuntime, PrintableError> {
    let context = Context::new();
    let mut test_runtime = DebugRuntime::new(&context, files.to_vec());
    {
        let mut codegen =
            CodeGen::compile(&context, &mut test_runtime, symbolizer, prog, true, dump)?;
        codegen.run();
    }
    assert_eq!(
        test_runtime.strings_in(),
        test_runtime.strings_out(),
        "LEFT strings in does not match RIGHT strings out. This program caused a memory leak."
    );
    Ok(test_runtime)
}
...
impl<'a, RuntimeT: Runtime> CodeGen<'a, RuntimeT> {
    fn compile(
        ...
        function_map.insert(
            main_sym.clone(),
            CallableFunction::main(main_function.clone(), main_function_typed),
        );
    ...
    fn compile_inner(
        ...
        for (name, parser_func) in functions.user_functions().iter() {
            ...
            FunctionCodegen::build_function(
                callable_function.jit_function().clone(),
                parser_func,
                self.runtime,
                &self.function_map,
                &mut self.context,
